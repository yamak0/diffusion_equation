...
void calc_diff(vector<double> &fluid_diff, vector<double> &solid_diff, onedimensinal_diffusion Fluid, onedimensinal_diffusion Solid)
{
    for(int i=0; i<fluid_diff.size(); i++){
        fluid_diff[i]=-0.2*(Fluid.access_c(i)-Solid.access_c(i));
        solid_diff[i]=-0.01*(Solid.access_c(i)-Fluid.access_c(i));
...
int main()
{
    ...
    phi_s = 0.8;
    phi_f = 0.2;
    onedimensinal_diffusion Fluid(div, 0.1, 0.1, 10), Solid(div, 0.1, 0.01, 0.01);
